#' @title Calculate the activity score of gene sets in cells with Restart Walk with Restart and
...
#' default is 0.4.
#' @param knn.used.reduction the name of reduction to be used to build KNN Graph, default is 
#' \code{MCA}.
...
#' @param sv.used.reduction character  default is \code{UMAP}.
...
#' @param run.sv logical whether run the identication of SV features, if it is \code{FALSE}, the identification of SV features 
...
setMethod('detect.svp', 
  ...
  if (!knn.used.reduction %in% reducedDimNames(data)){
      cli::cli_abort(c("The {.cls {class(data)}} does not have {knn.used.reduction},

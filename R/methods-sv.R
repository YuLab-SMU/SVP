...
#' da1 <- data.frame(kld = abs(rnorm(0.4, nrow(sce))), pvalue = abs(rnorm(0.001, nrow(sce)))) |> as.matrix() 
#' rownames(da1) <- rownames(sce)
#' da2 <- data.frame(moransi = abs(rnorm(0.4, nrow(sce))), pvalue = abs(rnorm(0.001, nrow(sce))))
